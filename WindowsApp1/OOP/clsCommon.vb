...
Public Class clsCommonMethods
    ...
    Public Property GetMyPatientID As String
        Get
            Return visitID
        End Get
        Set(ByVal value As String)
    ...
    Public Sub updateMyTableRecords(ByVal mytbl As String, ByVal myschema As String, ByVal myclmn As String, ByVal myvalue As String, ByVal vsID As Integer)
        ...
        Try
            openConn()
            Dim mysql As String = "UPDATE [elab].[" + myschema + "].[" + mytbl + "] SET " + myclmn + "='" + myvalue + "' WHERE status='NO' AND Visit_ID=" & GetMyPatientID
    ...
    Public Function validateControlsOnForml(ByVal frm As Form)
        Dim nud As NumericUpDown
        Dim chlstBox As CheckedListBox
        Dim cbo As ComboBox
        GetMyTabControl = frm
    ...
    Public Function validateControlsOnTabPage(ByVal tbp As TabPage)
        Dim nud As NumericUpDown
        Dim chlstBox As CheckedListBox
        Dim cbo As ComboBox
        Gettabpage = tbp
    ...
    'Public Sub GetMyDataTableValues(ByVal tbl As String, ByVal schma As String, ByVal clm As String, ByVal chlb As CheckedListBox, ByVal vsID As String)
    ...
    '    Try
    '        openConn()
    '        Dim myQuery As String = "SELECT " + GetMyColumnValue + " FROM [elab].[" + GetMySchema + "].[" + GetMyTable + "] WHERE barcode = '" & sBarcode & "'"
    ...
    Public Sub GetMyDataTableValues22(ByVal tbl As String, ByVal schma As String, ByVal clm As String, ByVal chlb As CheckedListBox, ByVal vsID As String)
        ...
        Try
            openConn()
            Dim myQuery As String = "SELECT " + GetMyColumnValue + " FROM [elab].[" + GetMySchema + "].[" + GetMyTable + "] WHERE labref= '" & sLabref.ToString & "' and '" + GetMyColumnValue + "' ! = ''"
            Dim mycmd As New SqlCommand(myQuery, Qconn)
            Dim mydr As SqlDataReader = mycmd.ExecuteReader
            If mydr.HasRows Then
                While mydr.Read
        ...
        '    For i As Integer = 0 To GetMydataListValues.Count - 1
        '        For j As Integer = 0 To GetMyCheckedkedListBox.Items.Count - 1
        '            ListCount = InStr("" & GetMydataListValues(i), "" & GetMyCheckedkedListBox.Items(j).ToString, CompareMethod.Text)
        '            If ListCount > 0 Then
        '                GetMyCheckedkedListBox.SetItemCheckState(j, CheckState.Checked)
        '            End If
        '        Next j
    ...
    Public Sub GetMyDataTableValues(ByVal tbl As String, ByVal schma As String, ByVal clm As String, ByVal chlb As CheckedListBox, ByVal vsID As String)
        ...
        Try
            ...
            End If
        Catch ex As Exception
            msgError("system error in mining sample data " + ex.Message)
            Exit Sub
            Return
        Finally
            closeConn()
    ...
    Public Sub ShowPCSMode(ByVal frm As Form)
        SkyWater.lblilabMode.Text = ilab_Mode
        Select Case ilab_Mode
            Case "Add"
                frm.BackColor = Color.DimGray
                SkyWater.lblilabMode.Text = "ADD"
                SkyWater.lblilabMode.BackColor = Color.LightGreen
            Case "Edit"
                frm.BackColor = Color.LightPink
                SkyWater.lblilabMode.Text = "EDIT Mode"
                SkyWater.lblilabMode.BackColor = Color.Red
        End Select
    End Sub
    ...
    '    End Try
    'End Sub
    Public Function validate_nud(ByVal nud As NumericUpDown, ByVal msg As String) As Boolean
        If nud.Enabled = True And nud.Value = 0 Then
            MessageBox.Show(msg, "labmaster: Missing Data", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            nud.BackColor = Color.Lavender
            Return False
        Else
            Return True
        End If
    ...
    Public Function GetMyCounts_eLIMS(ByVal category As String, ByVal myDay As String)
    ...
    '    Try
    '        openConn()
    '        Dim myQuery As String = "SELECT " + GetMyColumnValue + " FROM [elab].[" + GetMySchema + "].[" + GetMyTable + "] WHERE SPecimenID=" & sBarcode
    '        Dim mycmd As New SqlCommand(myQuery, Qconn)
    '        Dim mydr As SqlDataReader = mycmd.ExecuteReader
    '        If mydr.HasRows Then
    '            While mydr.Read
